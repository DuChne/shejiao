...
function is_html($str=''){
    if(!empty($str) && strlen($str) != strlen(strip_tags($str)))return true;
    return false;
}

function is_json($str=''){
    if (json_decode($str, true)) return true;
    else return false;
}
//------------------deng end quanju-------------------
...
function error_tz($url,$msg,$status = 4)
{
    global $_W;
    if ($_W['isajax'])
    {
        show_json($status, array('url' => $url, 'message' => $msg));
    }
    //--------------deng start is_login_app---------------------
    elseif ($_W['is_api']){
        show_json($status, $msg);
    }
    //---------------deng end is_login_app----------------------
    header('location: ' . $url);
    exit();
}
