    ...
    public function __construct()
    {
        ...
        $wap = m('common')->getSysset('wap');
         ...
         if (($this instanceof MobileLoginPage) || ($this instanceof PluginMobileLoginPage)) {
             ...
             //token验证
             if(!$_GPC['token']) error_tz($loginurl,'请登录token');
             if(!$_GPC['token'] or !($token = redis()->hGet('token_validate',$_GPC['token']))){
                error_tz($loginurl,'请登录token');
             }

             $token = explode(':',$token);
             //验证sessio过期时间
             if(isset($token[2]) and $token[2] < time()){
                 error_tz($loginurl,'身份已过期请重新登录');
             }

             $this->member = db()->name('ewei_shop_member')->where(['openid' => $token[0]])->field('id,credit1,credit2,mobile,openid')->find();
             if(!$this->member){
                 error_tz($loginurl,'验证失败');
             }
        ...
        if(!(empty($member))) $memberid = $member['id'];
        ...
        if (!(empty($diypagedata))) {
            ...
            if (!(empty($diypageid))) {
                ...
                if (!(empty($page))) {
                    ...
                    include $this->template('diypage',get_defined_vars());
                    exit();
                }
