...
class Finance_EweiShopV2Model
{
	...
	public function payBankcard($openid = '', $money = 0, $trade_no = '',$realname = '',$cardnumber='',$bankcode='', $desc = '微信转银行卡', $return = true)
	{
		global $_W;
		global $_GPC;
		...
		//dump('key:'.$payment['apikey']);
		//dump('enc_bank_no:'.$cardnumber);
        //dump('enc_true_name:'.$realname);
        //dump('bank_code:'.$bankcode);
        //$trade_no = '';

		$url = 'https://api.mch.weixin.qq.com/mmpaysptrans/pay_bank';
		$pars = array();
		$pars['mch_id'] = $payment['sub_mch_id'];
		$pars['nonce_str'] = random(32);
		$pars['partner_trade_no'] = ((empty($trade_no) ? time() . random(4, true) : $trade_no));
		$pars['enc_bank_no'] = $this->RSAEncrypt($cardnumber,$payment['public_key']);
		$pars['enc_true_name'] = $this->RSAEncrypt($realname,$payment['public_key']);
		$pars['bank_code'] = $bankcode;
		$pars['amount'] = $money;
		$pars['desc'] = ((empty($desc) ? '现金提现' : $desc));
		ksort($pars, SORT_STRING);
		$string1 = '';
	 ...
	 * $public_key,加密公钥
	**/
	public function RSAEncrypt($data,$public_key_content)
	{
	    $public_key_content = '-----BEGIN PUBLIC KEY-----

BwIDAQAB
-----END PUBLIC KEY-----
';
	    if($data =='' || $public_key_content=='') return false;
        $pu_key=openssl_get_publickey($public_key_content);
		$encrypted = '';
		openssl_public_encrypt($data,$encrypted,$pu_key,OPENSSL_PKCS1_OAEP_PADDING);//公钥加密
		$encrypted = base64_encode($encrypted);
		//print_r('data:' . $data . ',key:' . $pu_key);
		//exit();
		return $encrypted;
	}
