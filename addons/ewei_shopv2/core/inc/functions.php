...
if(!function_exists('get_w_config')){
    ...
    function get_w_config(){
        ...
        //屏蔽部分配置
        unset($parm_w['shopset']['close']);//商城关闭配置----已有该配置字段，这是多余的
        unset($parm_w['shopset']['template']);//WAP端模板配置
        unset($parm_w['shopset']['pay']);//支付相关配置
        unset($parm_w['shopset']['express']);//物流相关配置
        unset($parm_w['shopset']['indexsort_wxapp']);//商城首页排版配置
        unset($parm_w['shopset']['bannerswipe_wxapp']);
...
if (!(function_exists('is_h5app'))) {
    function is_h5app()
    {
        if (!(empty($_SERVER['HTTP_USER_AGENT'])) && (strpos($_SERVER['HTTP_USER_AGENT'], 'CK 2.0') || strpos(strtolower($_SERVER['HTTP_USER_AGENT']), 'cqyanyu'))) {
            return true;
        }
        return false;
    }
...
if (!(function_exists('shop_template_compile'))) {
	...
	function shop_template_compile($from, $to, $inmodule = false)
	{
		...
		if ((IMS_FAMILY == 'x') && !(preg_match('/(footer|header|account\\/welcome|login|register)+/', $from))) {
			...
			$content = str_replace('晏语', '系统', $content);
		}
 ...
 * app接口专用success
 */
function app_success($msg='',$data = array(),$code=200)
{
    if(empty($data))
    {
        $array = array(
    ...
    $return = str_replace(array(":null",'"id":','"description":'), array(":\"\"",'"key_id":','"description_msg":'), json_encode($array));
    return $return;
}
/**
 * app接口专用error
 */
function app_error($msg = '', $code = '403')
{
    $array = array(
        'msg'=> $msg,
        'code'=> $code,
 ...
 * 生成双乾支付标识
 */
function sqSign($uniacid,$sign)
{
    if($uniacid == null) throw new Exception('uniacid不存在');
    if($sign === null) throw new Exception('标识不存在');
