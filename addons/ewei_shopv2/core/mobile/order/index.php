...
class Index_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function get_list() 
	{
        ...
        foreach ($list as &$row )
        {
            ...
            $sql = 'SELECT coalesce(op.marketprice,g.marketprice) as marketprice,coalesce(op.costprice,g.costprice) as costprice,coalesce(op.productprice,g.productprice) as productprice,og.goodsid,og.total,g.title,g.cannotrefund,g.thumb,g.status,og.price,og.optionname as optiontitle,og.optionid,op.specs,g.merchid,og.seckill,og.seckill_taskid,' . "\r\n" . '                og.sendtype,og.expresscom,og.expresssn,og.express,og.sendtime,og.finishtime,og.remarksend' . "\r\n" . '                FROM ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_goods') . ' g on og.goodsid = g.id ' . ' left join ' . tablename('ewei_shop_goods_option') . ' op on og.optionid = op.id ' . ' where ' . $scondition . ' order by og.id asc';
            ...
            foreach ($goods as &$r )
            {
                ...
                if (!(empty($r['cannotrefund'])) && ($row['status'] == 2))
                {
                    $goodsrefund = false;
                }

                //获取商品总量
                $row['total_num'] += $r['total'];
            ...
            if ($ismerch)
            {
                ...
                foreach ($getListUser['merch'] as $k => $v )
                {
                    if (empty($merch_user[$k]['merchname']))
                    {
                        ...
                        $goods_list[$k]['mobile'] = $_W['shopset']['shop']['tel'];
                    }
                    else
                    {
                        $goods_list[$k]['shopname'] = $merch_user[$k]['merchname'];
                        $goods_list[$k]['mobile'] = $merch_user[$k]['mobile'];
            ...
            else
            {
                if ($merchid == 0)
                {
                    ...
                    $row['mobeil'] = $_W['shopset']['shop']['phone'];
                }
                else
                {
                    $merch_data = $merch_plugin->getListUserOne($merchid);
                    $row['mobeil'] = $merch_data['mobile'];
            ...
            if (!(empty($row['refundid'])))
            {
                ...
                if ((0 < $row['refundstate']) and !(empty($refund)))
                {
            ...
            $canrefund = false;
            /****************tan shen xiao 判断是否可以退货 start *******************/
            if (($row['status'] == 1) || ($row['status'] == 2))
            {
                $canrefund = true;
                if (($list['status'] == 2) && ($row['price'] == $row['dispatchprice']))
                {
	...
	public function detail() 
	{
        ...
        {
            $refund = pdo_fetch('select status from ' . tablename('ewei_shop_order_refund') . ' where id=:id and uniacid=:uniacid and orderid=:orderid limit 1', array(':id' => $order['refundid'], ':uniacid' => $uniacid, ':orderid' => $order['id']));
            if($refund){
        ...
        foreach ($getListUser['merch'] as $k => $v )
        {
            if (empty($merch_user[$k]['merchname']))
            {
                ...
                $goods_list[$i]['mobile'] = $_W['shopset']['shop']['tel'];
            }
            else
            {
                $goods_list[$i]['shopname'] = $merch_user[$k]['merchname'];
                $goods_list[$i]['mobile'] = $merch_user[$k]['mobile'];
