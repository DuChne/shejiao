...
class Info_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function submit() 
	{
		...
		{
			//$arr = array('realname' => trim($memberdata['realname']), 'weixin' => trim($memberdata['weixin']), 'zhifubao' => trim($memberdata['zhifubao']), 'birthyear' => intval($memberdata['birthyear']), 'birthmonth' => intval($memberdata['birthmonth']), 'birthday' => intval($memberdata['birthday']), 'province' => trim($memberdata['province']), 'city' => trim($memberdata['city']), 'area' => trim($memberdata['area']), 'datavalue' => trim($memberdata['datavalue']), 'mobile' => intval($memberdata['mobile']));
            if(!empty($memberdata['realname']))$arr['realname'] = trim($memberdata['realname']);
            if(!empty($memberdata['weixin']))$arr['weixin'] = trim($memberdata['weixin']);
            if(!empty($memberdata['zhifubao']))$arr['zhifubao'] = trim($memberdata['zhifubao']);
            if(!empty($memberdata['birthyear']))$arr['birthyear'] = intval($memberdata['birthyear']);
            if(!empty($memberdata['birthmonth']))$arr['birthmonth'] = intval($memberdata['birthmonth']);
            if(!empty($memberdata['birthday']))$arr['birthday'] = intval($memberdata['birthday']);
            if(!empty($memberdata['mobile'])){
                $arr['mobile'] = intval($memberdata['mobile']);
                if(!preg_match("/1[34578]{1}\d{9}$/", $arr['mobile']) || strlen($arr['mobile']) != 11)show_json(0,'请输入正确的手机号');
            }
    ...
    public function feedback()
    {
        ...
        $insert = pdo_insert('ewei_shop_feedback', $data, array('openid' => $_W['openid'],'uniacid'=>$_W['uniacid']));
        if($insert)show_json(1,'提交成功');
    }
