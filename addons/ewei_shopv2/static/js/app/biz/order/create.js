define(['core', 'tpl', 'biz/plugin/diyform'], function (core, tpl, diyform) {
    ...
    modal.submit = function (obj, token) {
        ...
        var liveid = parseInt($("#liveid").val());
		...
		if(modal.params.nosearch)
		{
			//效验一下地址选择了没有
			area = $('#areas').val();
			//再次效验一下区域是否已经有代理了
			var codes = area.split(' ');
			var area_code = codes[2];
			core.json('order/create/checkagentarea', {area:area_code}, function (ret) {
				if (ret.status == 0) {
					$this.attr('stop', 1);
					$('#areas').val();
			...
			realmobile = $('#realmobile').val();
			if($('#areas').isEmpty()){
				FoxUI.toast.show('请选择代理地区');
				return
			}
			if($('#realname').isEmpty()){
			...
			if($('#realmobile').isEmpty()){
				FoxUI.toast.show('请填写联系电话');
				return
			}
						
			remark = area + '-' + realname + '-' + realmobile;
		}
		else
		{
        ...
        var data = {
			...
			'nosearch':modal.params.nosearch
        };
