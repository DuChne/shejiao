...
class Bonus_EweiShopV2Page extends AbonusMobileLoginPage
{
	...
	public function get_list()
	{
		...
		foreach ($list as &$row ) {
			...
			$bill = pdo_fetch('select * from ' . tablename('ewei_shop_abonus_bill') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $row['billid'], ':uniacid' => $_W['uniacid']));
			if($bill) $row['payno'] ='' . $bill['year'] .'年' . $bill['month'] . '月';
		}
