...
class Sms_EweiShopV2ComModel extends ComModel 
{
	public function send($mobile, $tplid, $data, $replace = true) 
	{
		...
		{
		    //---------------deng start log_aliyunsms------------
		    //file_put_contents('log_aliyunsms.txt','aliyun:' . json_encode($params) . ',template:' . json_encode($template));
            //----------------deng end log_aliyunsms-------------
            load()->func('communication');
            ...
            //---------------deng start log_aliyunsms_curl------------
            //file_put_contents('aliyunsms2.txt','aliyun:' . 'http://sms.market.alicloudapi.com/singleSendSms?' . $paramstr . ',template:' . json_encode($header));
            //----------------deng end log_aliyunsms_curl-------------
