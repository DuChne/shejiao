...
class Task_EweiShopV2Page extends SeckillWebPage
{
    ...
    protected function post()
    {
        ...
        if ($_W['ispost'])
        {
            ...
            foreach($alltimes2 as $key => &$items){
                $end = round($ends[$items],2);
                if(!$end) show_json(0,$items.'点没有选择结束时间');
                if($end < 0 or $end >= 24) show_json(0,$items.'请选贼正确的结束时间');

                if($key != 0){
                    $up_end = round($ends[$items],2);
                    $up_time = (float)$alltimes[$key-1] + (float)$ends[$alltimes[$key-1]];

                    if($up_time > $items) show_json(0,$items.'上一个结束时间大于了这一个时间');
                }
            ...
            foreach ($alltimes as $i )
            {
                ...
                if (empty($time))
                {
                    $time = array('uniacid' => $_W['uniacid'], 'taskid' => $id, 'time' => $i,'endtime' => $ends[$i]);
                    pdo_insert('ewei_shop_seckill_task_time', $time);

                }else{

                    pdo_update('ewei_shop_seckill_task_time', array('endtime' => round($ends[$i],2)), array('id' => $time['id']));
