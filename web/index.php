...
//-----------deng start uniacid_account-------------
if (empty($_W['uniacid']) && $_W['uid']){
    $_W['isfounder'] = true;
    $_W['uniacid'] = empty($_GPC['__uniacid'])?2:$_GPC['__uniacid'];
    $role = uni_permission($_W['uid'], $_W['uniacid']);
    if (empty($role)) {
        message('操作失败, 非法访问.');
    }
    if (function_exists('uni_account_save_switch')) {
        uni_account_save_switch($_W['uniacid']);
    }
    isetcookie('__uniacid', $_W['uniacid'], 7 * 86400);
    isetcookie('__uid', $_W['uid'], 7 * 86400);
    $_W['uniaccount'] = $_W['account'] = uni_fetch($_W['uniacid']);
}
//------------deng end uniacid_account--------------
