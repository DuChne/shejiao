...
class Member_EweiShopV2Model 
{
	...
	public function upgradeLevel($openid, $orderid = 0) 
	{
		...
		{
		    $ordermoney = pdo_fetchcolumn('select ifnull( sum(og.realprice),0) from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_order') . ' o on o.id=og.orderid ' . ' where o.openid=:openid and (o.status>=1 and o.status<=3) and o.uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
			$level = pdo_fetch('select * from ' . tablename('ewei_shop_member_level') . ' where uniacid=:uniacid  and enabled=1 and ' . $ordermoney . ' >= ordermoney and ordermoney>0  order by level desc limit 1', array(':uniacid' => $_W['uniacid']));
		}
		if (!(empty($orderid))) 
		{
		...
		$selfinfo = pdo_fetch("select * from ".tablename('tiger_newhu_share')." where weid='{$_W['uniacid']}' and from_user='{$openid}'");
		if ($level['id'] == $member['level'] && !empty($selfinfo) && ($selfinfo['dltype']==1 && $selfinfo['tgwid'] !='' && $selfinfo['dlqqpid']!='' && $selfinfo['dlptpid'] !='')) 
		{
			return;
		}
		...
		//同步要检测是否成为优惠券经销商
		//同步要生成优惠券的代理商
		if($canupgrade || $member['isagent']==1)
		{
			//$selfinfo = pdo_fetch("select * from ".tablename('tiger_newhu_share')." where weid='{$_W['uniacid']}' and from_user='{$openid}'");
			if(!empty($selfinfo))
			{
			    //更新一下
				if($selfinfo['dltype']==1 && $selfinfo['tgwid'] !='' && $selfinfo['dlqqpid']!='' && $selfinfo['dlptpid'] !='')
				{
				}
				else
				{
				    $sharedata = array();
				    //标识上级ID
					if($selfinfo['helpid']>0)
					{
					}
					else
					{
					     $helpmember = pdo_fetch("select * from ".tablename('ewei_shop_member')." where uniacid='{$_W['uniacid']}' and id='{$member['agentid']}'");
						 if($helpmember)
						 {
							 $helpshare=pdo_fetch("select * from ".tablename('tiger_newhu_share')." where weid='{$_W['uniacid']}' and (from_user='{$helpmember['openid']}' OR shop_member_id='{$helpmember['id']}')");
							 if($helpshare) $sharedata['helpid'] = $helpshare['id'];
						 }
					}
					...
					if($selfinfo['tgwid'] =='')
					{
					     $tgw = pdo_fetch('select *  from ' . tablename('tiger_wxdaili_tgw') . ' where weid=:uniacid and uid=0 order by id asc', array(':uniacid' => $_W['uniacid']));
						 if($tgw && $tgw['tgw']!='')
						 {
						     $tgwarr = explode('_',$tgw['tgw']);
						     $sharedata['tgwid'] = $tgwarr[3];
							 $sharedata['dlqqpid'] = $tgw['tgw'];
							 $sharedata['dlptpid'] = $tgw['tgw'];
						 }
						 else
						 {
						     //TODO,发送通知（推广位不足)
					...
					if(!empty($sharedata))
					{
						 ...
						 pdo_update('tiger_newhu_share', $sharedata, array('from_user' => $openid));
						 pdo_update('tiger_wxdaili_tgw', array('uid'=>$member['id']), array('id' => $tgw['id']));
					}
					else
					{
					     //TODO,告知升级失败
						 //file_put_contents('memberupgradeerror.txt','memberid:' . $member['id'] . ',level:' . $level['id']);
					}					
				}
			}
			else
			{
			    //创建一条
				$sharedata = array();
				$sharedata['weid'] = $_W['uniacid'];
				$sharedata['openid'] = 0; //不在这里完善
				$sharedata['tel'] = $member['mobile'];
				$sharedata['type'] = 0;
				$sharedata['cqtype'] = 1;
				$sharedata['dltype'] = 1;
				$sharedata['dlbl'] = 40;
				$sharedata['zfbuid'] = $member['zhifubao'];
				$sharedata['tname'] = $member['realname'];
				$sharedata['weixin'] = $member['weixin'];
				$sharedata['from_user'] = $openid;
				$sharedata['nickname'] = $member['nickname'];
				$sharedata['avatar'] = $member['avatar'];
				$sharedata['score'] = 2;
				$sharedata['cscore'] = 1;
				$sharedata['pscore'] = 1;
				$sharedata['pid'] = 1;
				...
				$tgw = pdo_fetch('select *  from ' . tablename('tiger_wxdaili_tgw') . ' where weid=:uniacid and uid=0 order by id asc', array(':uniacid' => $_W['uniacid']));
				 ...
				 //写入数据
				 $insertid = pdo_insert("tiger_newhu_share",$sharedata);
				 pdo_update('tiger_wxdaili_tgw', array('uid'=>$member['id']), array('id' => $tgw['id']));
    ...
    public function checkMemberSNS($sns)
    {
        ...
        if ($sns == 'wx')
        {
            ...
            if(!is_h5app()){
                ...
                $appid = 'wxf9d4c382b24845f9';
                $secret = '848c9d394f2cdb5bdf0742725f119e3b';
        ...
        if (empty($member))
        {
            ...
            //----------deng start sns-------------------
            $data['nickname'] = empty($_GPC['nickname'])?'':$_GPC['nickname'];
            $data['avatar'] = empty($_GPC['avatar'])?'':$_GPC['avatar'];
            //-----------deng end sns--------------------
