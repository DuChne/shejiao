...
class Bind_EweiShopV2Page extends MobileLoginPage
{
	...
	public function main()
	{
		...
		if ($_W['ispost']) {
            ...
            if(!preg_match("/1[34578]{1}\d{9}$/", $mobile) || strlen($mobile) != 11)show_json(0,'请输入正确的手机号');
            ...
            //------------deng start bind_mobile_app----------
            $pwd1 = trim($_GPC['pwd1']);
            if(strlen(trim($pwd))<6)show_json(0,'密码长度最少6位');
            if(!empty($pwd1) && $pwd != $pwd1)show_json(0,'两次密码不一致');
            //-------------deng end bind_mobile_app-----------
