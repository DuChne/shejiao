...
//token生成sessionid
if($_GPC['token'] and ($token = redis()->hGet('token_validate',$_GPC['token']))){
    $token = explode(':',$token);
    if(isset($token[1])){
        $_W['session_id'] = $token[1];
    }
}
...
//---------------deng start is_api 定义常量变量IS_API，如果为true则禁用微信公众号功能-----------------
if(empty($_W['is_api']) && IS_API === true)$_W['is_api'] = IS_API;

if (!empty($_W['account']['oauth']) && $_W['account']['oauth']['level'] == '4' && IS_API !== true) {
...
if ($_W['container'] == 'wechat' && IS_API !== true) {
