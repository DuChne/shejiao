...
class Index_EweiShopV2Page extends PluginMobileLoginPage
{
	...
	public function detail()
	{
        ...
        $merch_user = $merch_plugin->getListUserOne($ewei_detail['merchid']);

        if (empty($merch_user))
        {
            $ewei_detail['shopname'] = $_W['shopset']['shop']['name'];
            $ewei_detail['mobile'] = $_W['shopset']['shop']['tel'];
        }
        else
        {
            $ewei_detail['shopname'] = $merch_user['merchname'];
            $ewei_detail['mobile']  = $merch_user['mobile'];
	...
	public function bargain()
	{
		...
		if ($ajax == 151) {
			...
			//tsx 加你还剩多少次
            $num = db('ewei_shop_bargain_record')->where(['openid' =>  $_W['openid'],'actor_id' => $res['id']])->count('id');
            $aiso_num = $res2['each_time'] - $num;
	...
	public function cut($actor_id, $time3, $min_price, $each_time, $total_time, $max_price, $probability_json)
	{
		...
		if ($now_price['now_price'] <= $min_price) {
            ...
            if(IS_API){
                show_json(0,['message' => '砍到底价了,快去告诉TA<br>可以按底价购买啦！']);
            }
		...
		if ($cut_price <= 0) {
            ...
            if(IS_API){
                show_json(1,['message' => '成功砍掉' . $cut_price . '元！']);
            }
		...
		if (0 < $cut_price) {
            ...
            if(IS_API){
                show_json(1,['message' => '难度增加' . $cut_price . '元！']);
            }
