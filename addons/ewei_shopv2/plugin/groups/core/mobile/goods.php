...
class Goods_EweiShopV2Page extends PluginMobileLoginPage 
{
	public function main() 
	{
        ...
        $getListUser = $merch_plugin->getListUser([$goods],'merch_user');
        if($getListUser){
            $goods['shopname'] = $getListUser['merchname'];
            $goods['mobile'] = $getListUser['mobile'];
        }else{
	...
	public function fightGroups() 
	{
		...
		foreach ($teams as $key => $value ) 
		{
			...
			$value['edn_time'] = ($value['paytime'] + ($value['endtime'] * 60 * 60));
