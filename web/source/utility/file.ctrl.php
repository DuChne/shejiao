...
if ($do == 'fetch' || $do == 'upload') {
	...
	//--------------deng start file_sql-------------------
    //数据库core_attachment的uid字段需改成varchar型
    $file_uid = empty($dest_dir)?$_W['uid']:$dest_dir;
...
if ($do == 'module') {
	...
	foreach ($installedmodulelist as $k => $value) {
		$installedmodulelist[$k]['official'] = empty($value['issystem']) && (strexists($value['author'], 'WeEngine Team') || strexists($value['author'], '晏语团队'));
	}
...
if ($do == 'image') {
	...
	if ($islocal) { 		$page = $_GPC['page'];
        ...
        //----------deng start file---------------
        session_start();
        $file_dest = empty($_SESSION['__dest_dir'])?$_GPC['__uid']:$_SESSION['__dest_dir'];
        $condition .= ' AND uid = :uid';
        $params[':uid'] = $file_dest;
        //-----------deng end file----------------
