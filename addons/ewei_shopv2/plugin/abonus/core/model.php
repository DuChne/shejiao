...
if (!(class_exists('AbonusModel'))) 
{
	class AbonusModel extends PluginModel 
	{
		...
		public function getBonusData($year = 0, $month = 0, $week = 0, $openid = '') 
		{
			...
			//获取总的优惠券结算记录数据
			//$sql = 'select y.*,m.openid,m.province,m.city,m.area from ' . tablename('tiger_wxdaili_yjlog') . ' y ' . ' inner join ' . tablename('ewei_shop_member') . ' m on y.openid = m.openid where m.area!="" and y.price>0 and y.weid=' . $_W['uniacid'] . ' and y.createtime >= ' . $starttime . ' and  y.createtime <=' . $endtime;			
			$jsorders = pdo_fetchall('select y.*,m.openid,m.province,m.city,m.area from ' . tablename('tiger_wxdaili_yjlog') . ' y ' . ' inner join ' . tablename('ewei_shop_member') . ' m on y.openid = m.openid where m.area!="" and m.isaagent=0 and y.price>0 and y.weid=' . $_W['uniacid'] . ' and y.createtime >= ' . $starttime . ' and  y.createtime <=' . $endtime);
			//$sql = 'select y.*,m.openid,m.province,m.city,m.area from ' . tablename('tiger_wxdaili_yjlog') . ' y ' . ' inner join ' . tablename('ewei_shop_member') . ' m on y.openid = m.openid where m.area!="" and m.isaagent=0 and y.price>0 and y.weid=' . $_W['uniacid'] . ' and y.createtime >= ' . $starttime . ' and  y.createtime <=' . $endtime;
			...
			unset($a);
			//计算订单的总金额和数量，并分别统计每个人应该结算的费用
			foreach ($orders as $o ) 
			{
				$ordermoney += $o['price'];
				//获取一下购买商品数量
				$ordergoods = pdo_fetch('select * from ' . tablename('ewei_shop_order_goods') . ' where orderid=:id limit 1', array(':id' => $o['id']));
				foreach ($aagents as &$a ) 
				{
					if (empty($set['selfbuy'])) 
					{
			...
			//结算优惠券应该分的费用(固定是10%的提佣)
			if($jsorders)
			{
			foreach ($jsorders as $o ) 
			{
				$jsordermoney += $o['price'];
				foreach ($aagents as &$a ) 
				{
					$bonusmoney1 = 0;
					$bonusmoney2 = 0;
					$bonusmoney3 = 0;
					$jsbonusmoney = round(($o['price'] * 10) / 100, 2);
					if ($a['aagenttype'] == 1) 
					{
						if (in_array($o['province'], $a['aagentprovinces'])) 
						{
							$bonusmoney1 = round(($o['price'] * 10) / 100, 2);
						}
						if (in_array($o['province'] . $o['city'], $a['aagentcitys'])) 
						{
							$bonusmoney2 = round(($o['price'] * 10) / 100, 2);
						}
						if (in_array($o['province'] . $o['city'] . $o['area'], $a['aagentareas'])) 
						{
							$bonusmoney3 = round(($o['price'] * 10) / 100, 2);
						}
					}
					else if ($a['aagenttype'] == 2) 
					{
					...
					$a['bonusmoney1'] += $bonusmoney1;     //省级分佣金额
					$a['bonusmoney2'] += $bonusmoney2;     //区级分佣金额
					$a['bonusmoney3'] += $bonusmoney3;     //县级分佣金额
					$a['jsbonusmoney'] += $jsbonusmoney;   //优惠券所占的分佣金额
				}
				unset($a);
			}
			}
			...
			unset($p);
			return array('orders' => $orders,'jsorders' => $jsorders, 'aagents' => $aagents, 'ordermoney' => round($ordermoney, 2), 'ordercount' => count($orders),'jsordermoney' => round($jsordermoney, 2), 'jsordercount' => count($jsorders), 'bonusmoney1' => round($bonusmoney1, 2), 'bonusmoney2' => round($bonusmoney2, 2), 'bonusmoney3' => round($bonusmoney3, 2), 'aagentcount1' => $aagentcount1, 'aagentcount2' => $aagentcount2, 'aagentcount3' => $aagentcount3, 'starttime' => $starttime, 'endtime' => $endtime, 'old' => false,'sql'=>$sql);
		}
