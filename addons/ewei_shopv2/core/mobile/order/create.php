...
class Create_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function main() 
	{
		...
		if (!($packageid)) 
		{
			...
			//if($_W['is_wx'] == 1)$couponcount += com_run('wxcard::consumeWxCardCount', $openid, $merch_array, $goodsdata_temp);
        ...
        //tsx_add 预载入判断是否可以购买
        if(isset($_GPC['validate']) and $_GPC['validate'] == 1) {
            show_json(1);
        }
		...
		$isagent = 0;
        if($member && $member['isagent'] == 1 && $member['status'] == 1)
        {
            $isagent = 1;
        }
		if($isagent!=1)
        ...
        {
            //读取一下 goods信息
            $goodsinfo = pdo_fetch('select nosearch from ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $goodsid, ':uniacid' => $_W['uniacid']));
            if($goodsinfo['nosearch']==1) $nosearch=1;
        }
        $createInfo['nosearch'] = $nosearch;
        ...
        unset($parm['log_id']);
        unset($parm['liveid']);
        unset($parm['bargain_id']);
        unset($parm['optionid']);
        unset($parm['buytotal']);
        unset($parm['errcode']);
        unset($parm['task_id']);
        unset($parm['merch_id']);
        unset($parm['open_redis']);
        unset($parm['seckillinfo']);
        unset($parm['allow_sale']);
        unset($parm['area_set']);
        unset($parm['merchdata']);
        unset($parm['is_openmerch']);
        unset($parm['merch_data']);
        unset($parm['merch_array']);
        unset($parm['merchs']);
        unset($parm['total_array']);
        unset($parm['level']);
        unset($parm['diyformdata']);
        unset($parm['iswholesale']);
        unset($parm['isvirtualsend']);
        unset($parm['fromcart']);
        unset($parm['carrier']);
        unset($parm['dispatch_list']);
        unset($parm['seckill_payprice']);
        unset($parm['gprice']);
        unset($parm['goodsdata']);
        unset($parm['goodsdata_temp']);
        unset($parm['g']);
        unset($parm['giftgoods']);
        unset($parm['mustbind']);
        unset($parm['buyagain']);
        unset($parm['isagent']);
        unset($parm['goodsinfo']);
        unset($parm['buyagain_sale']);
        unset($parm['goods']);
        unset($parm['threensql']);
        unset($parm['ishotelsql']);
        unset($parm['data']);
        unset($parm['threenprice']);
        unset($parm['follow']);
        unset($parm['rank']);
        unset($parm['task_goods_data']);
        unset($parm['deductprice']);
        unset($parm['deductprice2']);
        unset($parm['ismerch']);
        unset($parm['prices']);
	...
	public function submit()
	{
		...
		//加入区域总代判断
		$nosearch = intval($_GPC['nosearch']);
		if($nosearch==1)
		{
		    $remark = $_GPC['remark'];
			if($remark !='' && strpos($remark,'-'))
			{
			    $arrremark = explode('-',$remark);
				$chosearea = $arrremark[0];
				$choosedata = pdo_fetch('select id from ' . tablename('ewei_shop_order') . ' where remark like "%' . $chosearea . '%" and status>0 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
				if($choosedata)
				{
					show_json(0,'该区域已有人预定,请重新选择后提交');
				}
				$chosearea1 = str_replace(' ','',$chosearea);
				$choosedata = pdo_fetch('select id from ' . tablename('ewei_shop_member') . ' where aagentareas like "%' . $chosearea1 . '%" and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
				if($choosedata)
				{
					show_json(0,'该区域已有代理,请重新选择后提交');
				}
			}
		}		
		...
		foreach ($goods as $g ) 
		{
			...
			{
			    //tsx验证商品是否超出限购范围
                if(($data['seckillinfo']['selfcount'] + $g['total']) > $data['seckillinfo']['maxbuy']) {
                    show_json(0, '你已超出最大的购买范围');
                }
		...
		if (0 < $ismerch) 
		{
			$ordersn = m('common')->createNO('order', 'ordersn', 'ME'. sqSign(2,0));
		}
		else 
		{
			$ordersn = m('common')->createNO('order', 'ordersn', 'SH'. sqSign(2,0));
		}
		...
		if (!(is_null($_SESSION['exchangeprice'])) && !(empty($_SESSION['exchange'])) && p('exchange')) 
		{
			...
			$order['ordersn'] = m('common')->createNO('order', 'ordersn', 'DH'.sqSign(2,0));
		...
		else 
		{
			...
			foreach ($merch_array as $key => $value ) 
			{
				...
				$order['ordersn'] = m('common')->createNO('order', 'ordersn', $order_head.sqSign(2,0));
	...
	public function checkagentarea()
    {
        global $_W;
        global $_GPC;
        $area = $_GPC['area'];
        if(trim($area)=='') show_json(0,'参数为空');
        else
        {
            //读取一下数据库查询有没有数据
            $data = pdo_fetch('select id from ' . tablename('ewei_shop_member') . ' where aagentareas like "%' . $area . '%" and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
            if($data)
            {
                show_json(0,'该区域已有代理,请重新选择后提交');
            }
