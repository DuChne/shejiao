...
class Pay_EweiShopV2Page extends MobileLoginPage
{
    public function main()
    {
        ...
        {
            //---------------deng start pay_app-----------------
            if ($_W['is_api'])show_json(0,'该订单已失效');
            //----------------deng end pay_app------------------
            header('location: ' . mobileUrl('order'));
            exit();
        }
        ...
        if (is_h5app())
        {
            if(!empty($set['pay']['app_wechat'])){
                ...
                $conf=array(
                    'private_key'=>$sec['app_wechat']['apikey'],
                    'appid'=>$sec['app_wechat']['appid'],
                    'mch_id'=>$sec['app_wechat']['merchid'],
                    'attach'=>$_W['uniacid'] . ':0'
                );

                $appwxpay = m('app_wx_sign')->wxSignature($conf,$order['price'],$notify_url,$aliname,$log['tid']);
            }
            if(!empty($set['pay']['app_alipay']) && !empty($sec['app_alipay']['public_key'])){
                $aliconfig['private_key_path'] = m('common')->chackKey($sec['app_alipay']['private_key'], false);
                $aliconfig['partner'] = $sec['app_alipay']['appid'];
                $aliconfig['seller_id'] = $sec['app_alipay']['account'];
                $notify_url = $_W['siteroot'] . 'addons/ewei_shopv2/payment/alipay/notify.php';
                //$aliname = (empty($_W['shopset']['shop']['name']) ? $sec['app_wechat']['merchname'] : $_W['shopset']['shop']['name']);
                $aliname = $_W['uniacid'].':0:APP';
                //dump($aliconfig);
                //dump($notify_url);
                //dump($aliname);

                $appalipay = m('app_pay_sign')->paySignature($aliconfig,$log['tid'],$aliname,$notify_url,$order['price']);
            }
        ...
        $_W['siteurl'] = $_W['siteroot'].'app/index.php?i=2&c=entry&m=ewei_shopv2&do=mobile&r=returnPay&url='.$_GPC['returnUrl'];


        $payType = $_GPC['payType'] ? $_GPC['payType'] : 'pc';
        $returnUrl = $_W['siteurl'];
        $ordersn = $log['tid'];
        $price = $order['price'];
        $price = 0.01;
        $sqpay = new SqPay();
        $sqpay->pay($ordersn,$price,$returnUrl,$payType);
        ...
        $parm = array(
            ...
            'credit'        =>  $credit,
            'cash'          =>  $cash,
            'haveverifygood'=>  $haveverifygood,
            'appwxpay'      =>  empty($appwxpay)?'':$appwxpay,
            'appalipay'     =>  empty($appalipay)?'':$appalipay,
        );
    ...
    public function complete()
    {
        ...
        $type = $_GPC['type'];
        if (!(in_array($type, array('wechat', 'alipay', 'credit', 'cash','pc'))))
        {
            if ($_W['ispost'])
            {
                show_json(0, '未找到支付方式');
        ...
        //验证订单是否过期
        $validate_order = db('ewei_shop_order')->where(['uniacid' => $_W['uniacid'],'id' => $orderid])->field('status')->find();
        if($validate_order['status'] < 0){
            show_json(0,'订单已过期');
        }
        ...
        $payType = $_GPC['payType'] ? $_GPC['payType'] : 'pc';
        $returnUrl = $_GPC['returnUrl'] ? $_GPC['returnUrl'] : $_W['siteurl'];
        $ordersn = time();
        $price = 0.01;
        $sqpay = new SqPay();
        $sqpay->pay($ordersn,$price,$returnUrl,$payType);
