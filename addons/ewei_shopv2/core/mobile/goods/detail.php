	...
	public function main()
	{
		...
		if ($is_openmerch == 0) {
			...
			if (0 < $merchid) {
				...
				$err = true;
				include $this->template('goods/detail',get_defined_vars());
				exit();
			}
			...
			if (!(empty($address_list))) {
				...
				foreach ($areas['province'] as $value ) {
					...
					foreach ($value['city'] as $city ) {
						...
						foreach ((array)$city['county'] as $county ) {
        ...
        //---------------------deng start goods_lists-----------------
        $goods['shopshare'] = array('title' => (!(empty($goods['share_title'])) ? $goods['share_title'] : $goods['title']), 'imgUrl' => (!(empty($goods['share_icon'])) ? tomedia($goods['share_icon']) : tomedia($goods['thumb'])), 'desc' => (!(empty($goodsdesc)) ? $goodsdesc : $_W['shopset']['shop']['name']), 'link' => mobileUrl('goods/detail', array('id' => $goods['id']), true));
        //----------------------deng end goods_lists------------------
		$com = p('commission');
		...
		if (p('ccard') && ($goods['type'] == 20)) {
			...
			if ($diyform_plugin) {
				...
				if (!(empty($fields))) {
					...
					include $this->template('ccard/formfields',get_defined_vars());
			...
			include $this->template('ccard/ccard_detail',get_defined_vars());
			exit();
		}
		...
		if (p('ccard') && !(empty($commission_data['become_goodsid'])) && ($commission_data['become_goodsid'] == $goods['id'])) {
			...
			include $this->template('ccard/cmember_detail',get_defined_vars());
			exit();
		}
		...
		if ($plugin_diypage) {
			...
			if ($diypage) {
				...
				include $this->template('diypage/detail',get_defined_vars());
				exit();
			}
        ...
        unset($parm['levelid']);
        unset($parm['groupid']);
        unset($parm['dispatch_areas']);
        unset($parm['giftgoods']);
        unset($parm['shop']);
        unset($parm['goodsdesc']);
        unset($parm['param']);
        unset($parm['statics']);
        unset($parm['diypage']);
        unset($parm['cset']);
        ...
        unset($parm['mid']);
        unset($parm['merch_flag']);
        unset($parm['is_openmerch']);
        unset($parm['commission_data']);
        unset($parm['shopdetail']);
        unset($parm['opencommission']);
        unset($parm['merchid']);
        unset($parm['address_list']);
        unset($parm['showgoods']);
        unset($parm['task_goods_data']);
        unset($parm['is_task_goods']);
        unset($parm['bargain']);
        unset($parm['spec_titles']);
        unset($parm['canAddCart']);
        unset($parm['gflag']);
        unset($parm['goods_nofree']);
        unset($parm['goods_price']);
        unset($parm['isFavorite']);
        unset($parm['shop']);
        unset($parm['rank']);
        //------------deng end appjson_data------------
