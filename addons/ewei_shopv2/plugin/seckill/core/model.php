...
class SeckillModel extends PluginModel
{
    ...
    public function getSeckill($goodsid, $optionid = 0, $realprice = true, $openid = '')
    {
        ...
        foreach ((array)$times as $key => $time ) {
            ...
            //tsx_add
            $start_time = $times[$key]['time'];

            $starttime = mktime($start_time,0,0,date("m"),date("d"),date("Y"));
            $endtime = $starttime + (int)(($times[$key]['endtime'] * 60 * 60) -1);
    ...
    public function checkBuy($seckillinfo, $title, $unit = '')
    {
        ...
        if (0 < $seckillinfo['totalmaxbuy']) {
            ...
            if ($seckillinfo['totalmaxbuy'] <= $seckillinfo['selftotalcount']) {
                ...
                if (0 < $seckillinfo['selftotalnotpay']) {
                    ...
                    return error(-1, $title . ' 最多抢购 ' . $seckillinfo['totalmaxbuy'] . ' ' . $unit . ',  您有' . $seckillinfo['selftotalnotpay'] . '个未付款的，抓紧付款哦~');
                }
                ...
                return error(-1, $title . ' 您已经抢购 ' . $seckillinfo['totalmaxbuy'] . ' ' . $unit . '了哦，不能继续抢购了，看看别的吧!');
            }
        ...
        if (0 < $seckillinfo['maxbuy']) {
            ...
            if ($seckillinfo['maxbuy'] <= $seckillinfo['selfcount']) {
                ...
                return error(-1, $title . ' 您已经抢购 ' . $seckillinfo['maxbuy'] . ' ' . $unit . '了哦，不能继续抢购了，看看别的吧!');
            }
