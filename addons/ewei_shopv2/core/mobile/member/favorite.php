...
class Favorite_EweiShopV2Page extends MobileLoginPage 
{
    ...
    //	qtt begin collection
    public function togglecollection()
    {
        global $_W;
        global $_GPC;
        $id = intval($_GPC['id']);
        $iscollection = intval($_GPC['iscollection']);
        ...
        $data = pdo_fetch('select id,deleted from ' . tablename('ewei_shop_member_collection') . ' where uniacid=:uniacid and goodsid=:id and openid=:openid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid'], ':id' => $id));

        if (empty($data))
        {
            $data = array('uniacid' => $_W['uniacid'], 'goodsid' => $id, 'deleted'=>0,'merchid'=>$goods['merchid'],'type'=>0,'openid' => $_W['openid'], 'createtime' => time());
            pdo_insert('ewei_shop_member_collection', $data);
        }
        else
        {

            pdo_update('ewei_shop_member_collection', array('deleted' => $iscollection), array('id' => $data['id'], 'uniacid' => $_W['uniacid']));
        }
        show_json(1, array('iscollection' => $iscollection == 1));
    }
    public function get_listcolletion()
    {
        global $_W;
        global $_GPC;
        $merch_plugin = p('merch');
        ...
        $params = array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']);
        $sql = 'SELECT COUNT(*) FROM ' . tablename('ewei_shop_member_collection') . ' f where 1 ' . $condition;
        $total = pdo_fetchcolumn($sql, $params);
        $list = array();
        if (!(empty($total)))
        {
            $sql = 'SELECT f.id,f.goodsid,g.title,g.thumb,g.marketprice,g.productprice,g.merchid FROM ' . tablename('ewei_shop_member_collection') . ' f ' . ' left join ' . tablename('ewei_shop_goods') . ' g on f.goodsid = g.id ' . ' where 1 ' . $condition . ' ORDER BY `id` DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
            $list = pdo_fetchall($sql, $params);
            $list = set_medias($list, 'thumb');
            if (!(empty($list)) && $merch_plugin && $merch_data['is_openmerch'])
            {
        ...
        $total_pages = max(1,ceil($total/$psize));
        show_json(1, array('list' => $list, 'total' => $total, 'pagesize' => $psize,'pindex'=>$pindex,'total_pages'=>$total_pages));
    }
    //无分页
    public function get_listallcolletion()
    {
        global $_W;
        global $_GPC;
        $merch_plugin = p('merch');
        ...
        if (!(empty($total)))
        {
            $sql = 'SELECT f.id,f.goodsid,g.title,g.thumb,g.marketprice,g.productprice,g.merchid FROM ' . tablename('ewei_shop_member_collection') . ' f ' . ' left join ' . tablename('ewei_shop_goods') . ' g on f.goodsid = g.id ' . ' where 1 ' . $condition . ' ORDER BY `id` DESC';
            $list = pdo_fetchall($sql, $params);
            $list = set_medias($list, 'thumb');
            if (!(empty($list)) && $merch_plugin && $merch_data['is_openmerch'])
            {
