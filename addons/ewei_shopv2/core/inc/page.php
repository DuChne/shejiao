...
class Page extends WeModuleSite 
{
	...
	//---------------deng start template---------------------
	public function template($filename = '',$pram = array(), $type = TEMPLATE_INCLUDEPATH, $account = false)
    //----------------deng end template----------------------
	{
		global $_W;
		global $_GPC;
        //--------------deng start app_json--------------
        if(!empty($_W['is_api'])) $this->apiJson($pram);
        //---------------deng end app_json---------------
		if (empty($filename)) 
		{
        ...
        //----------deng start log---------
        //TODO HTML文件位置打印
        //echo "<script language='javascript'>console.log('%c 模板文件位置\\n{$source}','color:#449D44');</script>";
        //-----------deng end log----------
		if (!(is_file($source))) 
		{
        ...
        //----------deng start log---------
        //PHP文件位置打印 这个貌似是缓存文件，可不用理会
        //echo "<script language='javascript'>console.log('{$compile}');</script>";
        //-----------deng log end----------
		return $compile;
	}
     ...
     * @param array $parm
     */
	private function apiJson($parm = array(),$type = 1){
        global $_W;
        //公共参数
        if($type==1){
            //过滤重要的字段
            ...
            unset($parm['uniacid']);
            unset($parm['id']);
            unset($parm['php_errormsg']);
            unset($parm['err']);
            unset($parm['openid']);
            unset($parm['member']);
            unset($parm['_W']);
            $parm['_W'] = get_w_config();
        }
        ...
        foreach ($parm as $k=>$v){
            ...
            //过滤对象/SQL语句/SQL值
            if(is_object($v) || strpos($k,':') === 0 || (is_string($v) && (strpos($v,trim(tablename(),'`')) || strpos($v,'=:')) ) ){
                unset($parm[$k]);
                continue;
            }
            //string处理
            if(is_string($v)){
                if(is_serialized($v))$parm[$k] = unserialize($parm[$k]);//反序列化
                if(is_json($v))$parm[$k] = json_decode($v,true);//json转arr
                //如果未转换成array则执行string处理，反之则进入递归
                if(is_string($parm[$k])){
                    //实例化HTML标签
                    if(is_html($v))$parm[$k] = ihtmlspecialchars($v);
                    //elseifif($k != 'link' && strpos($v,'.php') && (!strpos($v,'http') || strpos($v,$_W['siteroot'])!==false))//将URL路径的index.php改成app.php，并拼接成全路径
                    elseif(strpos($v,'.php') && strpos($v,'./')===0){//将URL半路径的./index.php拼接成全路径
                        $v_url_arr = explode('.php',$v);
                        $v = $_W['sitescheme'].$_SERVER['HTTP_HOST'].$_W['script_name'].$v_url_arr[1];
                        $parm[$k] = $v;
                    }
                    //将图片路径转换为全路径
                    elseif(preg_match("/\w+\.(jpg|jpeg|gif|png)/",$v))$parm[$k] = tomedia($v);

                }

            }
            //递归多维数组
            if(is_array($v))$parm[$k] = $this->apiJson($v,2);

        }

        if($type==1)show_json(1,$parm);
