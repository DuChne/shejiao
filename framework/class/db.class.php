...
class DB {
	...
	public function debug($output = true, $append = array()) {
		...
		if($output) {
			...
			if (!empty($append['error'][1])) {
				...
				//-----------deng start sql pdo---------------
                //处理一下错误SQL，方便检查
				foreach ($append['params'] as $p_k=>$p_v){
				    $append['sql'] = str_replace($p_k,"'{$p_v}'",$append['sql']);
                }
                //------------deng end sql pdo----------------
				trigger_error("SQL Error: <br/>{$append['error'][2]}<br/>SQL: <br/>{$append['sql']}<br/>Params: <br/>{$params}<br/>SQL Error: <br/>{$append['error'][2]}<br/>Traces: <br/>{$ts}", E_USER_WARNING);
			}
		}
		return $this->errors;
	}
	...
	//------------------deng start sql pdo-------------------------
    /**
     * @TODO DCH 特地封装一个sql打印，方便查看并调试sql语句
     * @param bool $output
     * @param array $append
     * @return array
     */
    public function getlastsql($output = true, $append = array()) {
        if(!empty($append)) {
            $output = false;
            array_push($this->errors, $append);
        }
        if($output) {
            $data = end($this->errors);
            //处理一下SQL，方便检查
            foreach ($data['params'] as $p_k=>$p_v){
                $data['sql'] = str_replace($p_k,"'{$p_v}'",$data['sql']);
            }
            $re = $data['sql'];
