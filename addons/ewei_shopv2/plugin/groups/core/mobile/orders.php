...
class Orders_EweiShopV2Page extends PluginMobileLoginPage
{
	...
	public function detail()
	{
        ...
        $getListUser = $merch_plugin->getListUserOne($order['merchid']);
        if($getListUser){
            $order['shopname'] = $getListUser['merchname'];
            $order['mobile'] = $getListUser['mobile'];
        }else{
            $order['shopname'] = $_W['shopset']['shop']['name'];
            $order['mobile'] = $_W['shopset']['shop']['tel'];
        }
        ...
        {
            $refund = pdo_fetch('select refundstatus from ' . tablename('ewei_shop_groups_order_refund') . ' where id=:id and uniacid=:uniacid and orderid=:orderid limit 1', array(':id' => $order['refundid'], ':uniacid' => $uniacid, ':orderid' => $order['id']));
            if($refund){
                $order['refund_status'] = $refund['refundstatus'];
            }
        }
        //end
	...
	public function confirm()
	{
		...
		if ($goods['stock'] <= 0) {
            ...
            if(IN_API){
                show_json(0,['message' => '您选择的商品已经下架，请浏览其他商品或联系商家！']);
            }
		...
		if (!(empty($goods['purchaselimit'])) && ($goods['purchaselimit'] <= $ordernum)) {
            ...
            if(IN_API){
                show_json(0,['message' => '您已到达此商品购买上限，请浏览其他商品或联系商家！']);
            }
		...
		if ($order && ($order['status'] == 0)) {
            ...
            if(IN_API){
                show_json(0,['message' => '您的订单已存在，请尽快完成支付！']);
            }
		...
		if ($order && ($order['status'] == 1)) {
            ...
            if(IN_API){
                show_json(0,['message' => '您已经参与了该团，请等待拼团结束后再进行购买！']);
            }
		...
		if ($order && ($order['groupnum'] <= $ordernum)) {
            ...
            if(IN_API){
                show_json(0,['message' => '该团人数已达上限，请浏览其他商品或联系商家！']);
            }
		...
		if (!(empty($teamid))) {
			...
			foreach ($orders as $key => $value ) {
				...
				if ($orders && ($value['success'] == -1)) {
                    ...
                    if(IN_API){
                        show_json(0,['message' => '该活动已过期，请浏览其他商品或联系商家！']);
                    }
				...
				if ($orders && ($value['success'] == 1)) {
                    ...
                    if(IN_API){
                        show_json(0,['message' => '该活动已结束，请浏览其他商品或联系商家！']);
                    }
			...
			if ($num == $goods['groupnum']) {
                if(IN_API){
                    show_json(0,['message' => '该活动已成功组团，请浏览其他商品或联系商家！']);
                }
		...
		$ordersn = m('common')->createNO('groups_order', 'orderno', 'PT'.sqSign(2,5));

        //返回商家信息
        $merch_plugin = p('merch');
        $merch = $merch_plugin->getListUserOne($goods['merchid']);
        if($merch){
            $goods['shopname'] = $merch['merchname'];
            $goods['mobile'] = $merch['mobile'];
        }else{
            $goods['shopname'] = $_W['shopset']['shop']['name'];
            $goods['mobile'] = $_W['shopset']['shop']['tel'];
        }
		...
		if ($_W['ispost']) {
			...
			if ($isverify) {
				...
				if (empty($_GPC['realname']) || empty($_GPC['mobile'])) {
                    ...
                    if(IN_API){
                        show_json(0,['message' => '联系人或联系电话不能为空']);
                    }
            ...
            if(IN_API){
                show_json(1,['teamid' => (empty($teamid) ? $order['teamid'] : $teamid),'orderid' => $orderid,'message' => '成功']);
            }
