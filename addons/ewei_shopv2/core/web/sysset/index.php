...
class Index_EweiShopV2Page extends WebPage 
{
	...
	public function settemplateid() 
	{
		...
		if ($issnoet) 
		{
			...
			if (!(empty($result['errcode']))) 
			{
				...
				{
					show_json(1, array('status' => 0, 'messages' => '微信证书无效，请检查晏语access_token设置', 'tag' => $tag));
				}
				else 
				{
					show_json(1, array('status' => 0, 'messages' => $result['errmsg'], 'tag' => $tag));
	...
	public function payset() 
	{
		...
		if ($_W['ispost']) 
		{
            ...
            //----------deng start app_ali_pay_sys-------------
            $sec['app_alipay']['account'] = trim($_GPC['data']['app_alipay_account']);
            //-----------deng end app_ali_pay_sys--------------
	...
	public function contact() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$data['wxservice'] = trim($data['wxservice']);
			m('common')->updateSysset(array('contact' => $data));
			$shop = m('common')->getSysset('shop');
			$shop['qq'] = $data['qq'];
			$shop['address'] = $data['address'];
			$shop['phone'] = $data['phone'];
			$shop['wxservice'] = $data['wxservice'];
			m('common')->updateSysset(array('shop' => $shop));
