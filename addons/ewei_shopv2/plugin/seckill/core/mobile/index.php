...
class Index_EweiShopV2Page extends PluginMobilePage
{
    public function main()
    {
        ...
        $times = array();
        $validtimes = array();
        $day_end = mktime(23,59,59,date("m"),date("d"),date("Y")); //当天结束时间
        foreach ($alltimes as $key => $time )
        {
            ...
            $start_time = $alltimes[$key]['time'];

            $start_time = mktime($start_time,0,0,date("m"),date("d"),date("Y"));

            $endtime = $start_time + (int)(($alltimes[$key]['endtime'] * 60 * 60) -1);
        ...
        foreach ($validtimes as $key => $time )
        {
            ...
            $start_time = $validtimes[$key]['time'];

            $starttime = mktime($start_time,0,0,date("m"),date("d"),date("Y"));

            $endtime = $starttime + (int)(($validtimes[$key]['endtime'] * 60 * 60) -1);
        ...
        if ($plugin_diypage)
        {
            ...
            if ($diypage)
            {
                ...
                include $this->template('diypage/seckill',get_defined_vars());
                exit();
            }
        }
    ...
    public function get_goods()
    {
        ...
        $starttime = mktime($start_time,0,0,date("m"),date("d"),date("Y"));
        $endtime = $starttime + ($time['endtime'] * 60 * 60);
        ...
        $time['starttime'] = $starttime;
        // $endtime = strtotime(date('Y-m-d H:59:59', $starttime));
        ...
        if (!(empty($nexttime)) && time()>$endtime)
        {
            $end = $nexttime['time'] - 1;
            $endtime = strtotime(date('Y-m-d ' . $end . ':59:59'));
        }
        ...
        if(!empty($merchid)){
            $goods = $this->filterarr($goods,$merchid,'merchid');
        }
     ...
     * @auther qtt
     */
    public function filterarr($arr,$merchid,$param)
    {
        $new_arr = array();
        foreach ($arr as $val)
        {
            if ($val[$param] == $merchid)
                $new_arr[] = $val;
        }
        return $new_arr;
    }
