...
class Cart_EweiShopV2Page extends MobileLoginPage 
{
		...
		foreach ($list as &$g )
		{
			...
			if ($g['selected']) 
			{
				...
				if ($seckillinfo && ($seckillinfo['status'] == 0)) 
				{
                    ...
                    //如果秒杀的数量 比购物车的总数量小就更行数据
                    if($g['total'] > $seckilllast){
					    db('ewei_shop_member_cart')->where(['id' => $g['id']])->update(['total' => $seckilllast]);
					    $g['total'] = $seckilllast;
                    }
    ...
    public function select()
    {
        ...
        $merchid = array_key_exists('merchid',$_GPC)?$_GPC['merchid']:'a';
        if (!(empty($id)))
        {
        ...
        elseif ($merchid != 'a' && is_numeric($merchid)){
            pdo_update('ewei_shop_member_cart', array('selected' => $select), array('merchid' => $merchid, 'uniacid' => $_W['uniacid']));
        }
        else
        {
